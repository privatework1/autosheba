...
@include('site.inc.left-sidebar')
  ...
  <div class="section-list">
    ...
    @if(count($findbrandItem)>0)
      @foreach ($findbrandItem as $brandItem)
    <div class="sectls"> <a href="{{ url('/itemdetails').'/'.$brandItem->id }}" class="sectls-img">
        <img src="{{ url('/uploads/ItemImages').'/'.$brandItem->item_images[0] }}" alt="item Image.jpg" alt=""> </a>

      <div class="sectls-cont">
        <div class="sectls-ttl-wrap">
          <p><a href="#">Helmets</a></p>
          <h3><a href="{{ url('/itemdetails').'/'.$brandItem->id }}">{{ $brandItem->item_name }}</a></h3>
        </div>
        <div class="sectls-price-wrap">
          <p>Price</p>
          <p class="sectls-price">${{ $brandItem->sale_price }}</p>
        </div>

        <div class="sectls-qnt-wrap">
          <p>Code</p>
          <p class="sectls-price">{{ $brandItem->model_no }}</p>

        </div>

        <div class="sectls-qnt-wrap">
          <p>Size</p>
          <p class="sectls-price">{{ $brandItem->item_size }}</p>
      ...
      <div class="sectls-info">
        ...
        <p class="sectls-id"> &nbsp; &nbsp; &nbsp;id {{$brandItem->model_no}}</p>
        <p class="sectls-id">

        <p class="sectls-add">Total : ${{ $brandItem->sale_price }}</p>
        </p>
        <p class="sectls-add"> <a href="{{ url('/itemdetails').'/'.$brandItem->id }}">Add to cart</a> </p>
